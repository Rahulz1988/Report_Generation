...
//         let overallScore = parseFloat(row["Overall Score (Max. 75)"]); // Convert to number
//         let finalDegree = row["Final Degree"];
        
//         if (!isNaN(overallScore) && cutoffMap[finalDegree] !== undefined) {
//             row["Status"] = overallScore < cutoffMap[finalDegree] ? "R" : "P";
//         }
...
function applyCutoff() {
    ...
    excelData = excelData.map(row => {
        ...
        // If Status is still empty (undefined or blank), set it as "A"
        if (!row["Status"]) {
            row["Status"] = "A";
        }
